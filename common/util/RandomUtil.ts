...
export class RandomUtil {
    ...
    public static randomString(length: number = 10): string {
        ...
        for (let i = 0; i < length; i++) value += RandomUtil.RANDOM_STRING_VALUES.charAt(RandomUtil.randomNumber(0, maxLength - 1));
        return value;
    }
    ...
    public static randomNumber(min: number = NaN, max: number = NaN): number {
        if (isNaN(min)) min = 0;
        if (isNaN(max)) max = Number.MAX_VALUE - 1;
    ...
    public static randomArrayItem(array: Array<any>): any {
