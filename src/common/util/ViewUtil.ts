...
export class ViewUtil {
    ...
    public static getWidth(container: HTMLElement): number {
        ...
        if (isNaN(value)) value = container.offsetWidth;
        return value;
    }
    ...
    public static getHeight(container: HTMLElement): number {
        ...
        if (isNaN(value)) value = container.offsetHeight;
        return value;
    }
    ...
    public static setX(container: HTMLElement, value: number): void {
        if (container && !isNaN(value)) ViewUtil.setStyle(container, 'left', value + 'px');
    }
    ...
    public static setY(container: HTMLElement, value: number): void {
        if (container && !isNaN(value)) ViewUtil.setStyle(container, 'top', value + 'px');
    }
    ...
    public static focusInput(input: HTMLInputElement | HTMLTextAreaElement): void {
        let caretIndex = 0;
        if (input.value) caretIndex = Math.max(0, input.value.toString().length);
    ...
    public static addClass(container: any, name: string): void {
        ...
        if (container) ViewUtil.RENDEDER.addClass(container, name);
    }
    ...
    public static addClasses(container: any, names: string): void {
        ...
        container = ViewUtil.parseElement(container);
        if (container) names.split(' ').forEach(name => ViewUtil.RENDEDER.addClass(container, name));
    }
    ...
    public static removeClass(container: any, name: string): void {
        ...
        container = ViewUtil.parseElement(container);
        if (container) ViewUtil.RENDEDER.removeClass(container, name);
    }
    ...
    public static removeClasses(container: any, names: string): void {
        ...
        container = ViewUtil.parseElement(container);
        if (container) names.split(' ').forEach(name => ViewUtil.RENDEDER.removeClass(container, name));
    }
    ...
    public static toggleClass(container: any, name: string, value: boolean): void {
        if (value) ViewUtil.addClass(container, name);
        else ViewUtil.removeClass(container, name);
    }
    ...
    public static setProperty(container: any, name: string, value: any): void {
        ...
        container = ViewUtil.parseElement(container);
        if (container) ViewUtil.RENDEDER.setProperty(container, name, value);
    }
    ...
    public static removeAttribute(container: any, name: string): void {
        ...
        container = ViewUtil.parseElement(container);
        if (container) ViewUtil.RENDEDER.removeAttribute(container, name);
    }
    ...
    public static setAttribute(container: any, name: string, value: any): void {
        ...
        container = ViewUtil.parseElement(container);
        if (container) ViewUtil.RENDEDER.setAttribute(container, name, value);
    }
    ...
    public static setStyle(container: any, name: string, value: any): void {
        ...
        container = ViewUtil.parseElement(container);
        if (container) ViewUtil.RENDEDER.setStyle(container, name, value);
    }
    ...
    public static createVideo(isMute: boolean = true, isInline: boolean = false): HTMLVideoElement {
        ...
        if (isMute) ViewUtil.setVideoMuteParameters(video, isMute);
        if (isInline) ViewUtil.setVideoInlineParameters(video);

        return video;
    }

    public static setVideoMuteParameters(video: HTMLVideoElement, isMute: boolean = true): void {
        if (video) video.muted = video.defaultMuted = isMute;
    }
    ...
    public static stopVideoIfNeed(video: HTMLVideoElement): void {
        if (video && (video.src || video.srcObject)) ViewUtil.stopVideo(video);
    }
    ...
    public static disposeVideos(container: HTMLElement): void {
        for (let i = container.children.length - 1; i >= 0; i--) {
            ...
            if (item instanceof HTMLVideoElement) ViewUtil.disposeVideo(item);
        }
    }
    ...
    public static disposeObjects(container: HTMLElement, isIEBrowser?: boolean): void {
        for (let i = container.children.length - 1; i >= 0; i--) {
            let item = container.children.item(i);
            if (item instanceof HTMLObjectElement) ViewUtil.disposeObject(item, isIEBrowser);
        }
    }
