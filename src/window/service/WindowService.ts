...
import { WindowProperties } from '../lib/WindowProperties';
...
export class WindowService {
    ...
    private hasWindowWithSamePosition(itemWindow: IWindow): boolean {
        ...
        let result = false;
        this._windowsArray.forEach(window => {
            if (window !== itemWindow && x === window.getX() && y === window.getY()) result = true;
        });
        return result;
    }
    ...
    public info(translationId?: string, translation?: any): IQuestion {
        ...
        let question: IQuestion = content as IQuestion;
